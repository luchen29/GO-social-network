...
const (
	...
	POST_TYPE           = "post"
	DISTANCE            = "200km"
	ES_URL              = "http://35.247.75.209:9200"
	BUCKET_NAME         = "luchen-around-pj"
	BIGTABLE_PROJECT_ID = "keen-ally-241502"
...
func main() {
	...
	//http.HandleFunc("/search", handlerSearch) // Add this line
...
func saveToBT(p *Post, id string) error {
	...
	// create a new table called post
	tbl := bt_client.Open("post")
	// set a new row
	mut := bigtable.NewMutation()
	// set timestamp as NOW
	t := bigtable.Now()
	// convert to byte for storing in BT
	mut.Set("post", "user", t, []byte(p.User))
	mut.Set("post", "message", t, []byte(p.Message))
	mut.Set("location", "lat", t, []byte(strconv.FormatFloat(p.Location.Lat, 'f', -1, 64)))
	mut.Set("location", "lon", t, []byte(strconv.FormatFloat(p.Location.Lon, 'f', -1, 64)))
	//write the mutation (row) into table
